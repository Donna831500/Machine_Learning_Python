...
class relu:
    ...
    def forward(self, X):
        ...
        forward_output[forward_output < 0] = 0
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        derivative = np.ones(X.shape, dtype=float)-(np.tanh(X)**2)
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha <= 0.0:
                    ...
                    ####################################################################################
                    w_old = module.params[key]
                    w_new = w_old-(_learning_rate*g)
                    module.params[key] = w_new
                    ...
                    ###################################################################################################
                    w_old = module.params[key]
                    v = momentum[module_name + '_' + key]*_alpha-(_learning_rate*g)
                    w_new = w_old+v
                    module.params[key] = w_new 
